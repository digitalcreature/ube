    ...
    return struct {
        ...
        fn isFloat(comptime T : type) bool {
            return switch(@typeInfo(T)) {
                .Float => true,
                .Int => false,
                else => @compileError("only numeric element types allowed")
            };
        }
        ...
        fn VectorMixin(comptime Self: type, comptime size : usize) type {
            return struct {
                pub const Component = Real;
                pub const compound_type : CompoundType = .Vector;
                pub const is_float = isFloat(Real);
                pub const dimensions = size;
                pub const component_count = size;
                /// casts to other vector of the same width, but different type using @as
                pub fn cast(self: Self, comptime Target : type) Target {
                    const selfFields = @typeInfo(Self).Struct.fields;
                    const targetFields = @typeInfo(Target).Struct.fields;
                    var result : Target = undefined;
                    if (selfFields.len != targetFields.len) {
                        @compileError("cannot cast vectors of differing lengths!");
                    }
                    inline for (selfFields) |fld, i| {
                        const target_fld = targetFields[i];
                        if (is_float == Target.is_float) {
                            @field(result, target_fld.name) = @as(Target.Component, @field(self, fld.name));
                        }
                        else {
                            if (is_float) {
                                @field(result, target_fld.name) = @floatToInt(Target.Component, @field(self, fld.name));
                            }
                            else {
                                @field(result, target_fld.name) = @intToFloat(Target.Component, @field(self, fld.name));
                            }
                        }
                    }
                    return result;
                ...
                pub inline fn get(self : Self, comptime i : comptime_int) Component {
                    return comptime switch(i) {
                        0 => self.x,
                        1 => self.y,
                        2 => self.z,
                        3 => self.w,
                ...
                pub inline fn set(self : *Self, comptime i : comptime_int, value : Component) void {
                    comptime switch(i) {
                        0 => self.*.x = value,
                        1 => self.*.y = value,
                        2 => self.*.z = value,
        ...
        pub const Vec2 = extern struct {
            ...
            usingnamespace VectorMixin(Self, 2);
        ...
        pub const Vec3 = extern struct {
            ...
            usingnamespace VectorMixin(Self, 3);
        ...
        pub const Vec4 = extern struct {
            ...
            usingnamespace VectorMixin(Self, 4);
        ...
        fn MatMixin(dims : comptime_int) type {
            return struct {
                pub const Component = Real;
                pub const dimensions = dims;
                pub const component_count = dims * dims;
                pub const compound_type : CompoundType = .Matrix;
            };
        }
